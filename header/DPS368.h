...
class DPS368 : public gpio {
    public:
        ...
        bool readCalibrated_ifnew(double& pressure_Pa, double& temperature_C)
        {
            ...
            // Apply scaling
            double Praw_sc = (double)Praw / pressure_scale_factor_;
            double Traw_sc = (double)Traw / temp_scale_factor_;
        
            // Temperature (C)
            temperature_C = coeffs_.c0 * 0.5f + coeffs_.c1 * Traw_sc + 27.0;
        ...
        bool readCalibrated(double& pressure_Pa, double& temperature_C)
        {
            int32_t Praw, Traw;
            if (!readRaw(Praw, Traw))
                return false;
        ...
        double calculateAltitude(const double pressure, const double temp){
            ...
            double T = temp + 273.15; // convert temp from Celsius to Kelvin 

            return (R * T) / (g * M) * std::log(P0 / pressure);
         }
