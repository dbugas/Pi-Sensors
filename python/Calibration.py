...
def fit_ellipsoid(points):
    ...
    def ellipsoid_cost(p, points):
        ...
        # x^T Q x normalization
        pts = np.vstack((X, Y, Z))              # shape (3, N)
        Qp = Q @ pts
        norms_sq = np.sum(pts * Qp, axis=0)
        norms = np.sqrt(np.maximum(norms_sq, 1e-12))

        # Scaled residual (approx geometric distance)
        r = quad / norms
        r = np.clip(r, -1e6, 1e6)

        # Huber loss
        loss = huber_loss(r, 100)

        # Mean loss + regularization
        return np.mean(loss) + 1e-5 * np.sum(p**2)
...
def Get_data(imu, GET_MAGNETOMETER_DATA, GET_ACCELEROMETER_DATA):
    ...
    if(GET_ACCELEROMETER_DATA):
        print("Accelerometer - Press SPACE to stop")
        print("Press w to toggle data collection")
        print("-----------------------------------------\n")
...
def huber_loss(r, delta):
    ...
    loss[quadratic] = 0.5 * r[quadratic]**2
    loss[linear] = delta * (abs_r[linear] - 0.5 * delta)
